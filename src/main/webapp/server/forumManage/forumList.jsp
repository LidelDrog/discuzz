	...
	<script>
		...
		function getlist(forumId, title, isEffect) {
			tableIns = table.render({
				...
				url : '${ctx}/server/forum/getForumList',
				cols : [ [ {
					field : 'forumId',
					title : '编号',
					width : 80,
		...
		table.on('tool(forum)', function(obj) {
			...
			if (obj.event === 'updateForum') {
				var index = layui.layer.open({
					title : "编辑帖子",
					type : 2,
					content : "${ctx}/server/forum/goUpdateForum?forumId="
							+ data.forumId,
				...
				layer.confirm('确定禁用该条帖子吗?', function(index){
			        ...
			        $.ajax({
						type : "post",
						url : "${ctx}/server/forum/deleteForum",
						dataType : "json",
						data : {
				...
				var index = layui.layer.open({
					title : "查看评论",
					type : 2,
					content : "${ctx}/server/reply/goForumReplyList?forumId="
							+ data.forumId,
					success : function(layero, index) {
						setTimeout(function() {
							layui.layer.tips('点击此处返回',
